...
// gives a random list of colors

const colors = [];
data.forEach((data_obj, i) => {
  colors.push(data_obj.hexString);
});
...
function getRandomColorsList(){
  let list_of_colots = [];
  for(let i = 0;i<5;i++){
    list_of_colots[i] = colors[Math.floor(Math.random()*(colors.length))]
  }

  return list_of_colots;
}
